...
params_filepath = os.path.join(os.path.dirname(__file__), 'params-cruze.yaml')
...
# Transformação de rotação do motor para velocidade na marcha 6
vel_6 = [((math.pi / 30) * (1 - e) * r_d / (i_6 * i_d)) * x for x in rpm]
...
vel_kmh_6 = [3.6 * x for x in vel_6]

# Todas velocidades em um array
vel = sorted(list(set([*vel_1, *vel_2, *vel_3, *vel_4, *vel_5, *vel_6])))
...
plt.plot(rpm, vel_kmh_6, label='Velocidade na marcha 6 [km/h]')
plt.xlabel('RPM')
plt.legend()
...
plt.plot(vel_kmh_6, power_after_transm_rpm, label='Potência na marcha 6 [kW]')
...
plt.plot(vel_kmh_6, power_after_transm_rpm, label='Potência na marcha 6 [kW]')
plt.plot(vel_kmh, Pr_Pa_vel, label='Pa + Pr [kW]')
plt.vlines(211.5, 0, 300, linestyles="dashed")
plt.text(215, 150,'Vmax = 211.5 km/h',rotation=90)
plt.xlabel('Velocidade [km/h]')
plt.legend()
